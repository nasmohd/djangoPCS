...
from PCS_system.models import Student, Project, Role, Permission
...
def login_submit (request):
	...
	try:
		...
		if ((student.password == password) and (student.status != '0')): #Login accepted
			...
			user_permissions = Permission.objects.filter (role_id = student.role_id)
			status.append(0)
			for i in user_permissions:
				permissions.append (i.permission_name)
...
def user_info (request):
	...
	for k in roles_id:
		...
		role_permission_found = Permission.objects.filter(role_id = role) #[<Permission: 1, dashboard>, <Permission: 2, add users >]

		for l in role_permission_found:
			perms.append (l.permission_name)
	...
	return render (request, 'user_info.html', {'Users': users, 'user_permissions': permission, 'all_permission': all_permission, 'roles': roles,
		'count_permissions': count_permissions, 'role_permission': role_permission, 'roles_id': roles_id, 'dicti': dicti})
...
def manage_role(request):
	if request.POST['delete_role'] == 'delete':
	...
	if request.POST['update_role'] == 'update':
		return HttpResponseRedirect('user_info')
